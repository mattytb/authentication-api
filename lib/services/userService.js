...
import { getUserByAuthorizationToken, deleteAllClaimsByClaimantId } from '../clients/claimClient';
    ...
    return new Promise((resolve, reject) => {
        ...
        getUserByAuthorizationToken(authorizationToken).then(user => {
            if(userIdToBeDeleted == user._id || user.admin) {
                Promise.all([deleteUserById(userIdToBeDeleted), deleteAllClaimsByClaimantId(userIdToBeDeleted)])
                .then(resultArray => {
                    resolve(`${resultArray[0]} User was successfully deleted`)
                })
                .catch(err => reject(err))
            }
            else {
